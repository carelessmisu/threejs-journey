...
float random(vec2 st) {
    return fract(sin(dot(st.xy, vec2(12.988, 78.233))) * 43578.543123);
}
...
void main() {
    ...
    // pattern 30
    // float strength = 0.03 / (distance(vec2(vUv.y, (vUv.x - 0.5) * 0.5 + 0.5), vec2(0.5)));

    // pattern 31
    // float strength = 0.06 / (distance(vec2(vUv.y, (vUv.x - 0.5) * 0.5 + 0.5), vec2(0.5)));
    // strength *= 0.06 / (distance(vec2(vUv.x, (vUv.y - 0.5) * 0.5 + 0.5), vec2(0.5)));
